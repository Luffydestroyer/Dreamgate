...
func _process(_delta):
	...
	if next_item:
		...
		if i is DialogueFunction:
			...
			printerr("You accidently added a DE Resource or something.")
...
func _choice_resource(i: DialogueChoice):
	...
	if i.speaker_img:
		...
		SpeakerSprite.frame = min(i.speaker_img_select_frame, i.speaker_image_Hframes-1)
	else:
		$HBoxContainer/Speaker.visible = false
	$"HBoxContainer/VBoxContainer/Button-Hoarder".visible = true
	...
	for item in i.choice_text.size():
		...
		if function_resource:
			...
			Callable(get_node(function_resource.target.path), function_resource.function_name).bindv(function_resource.function_arguments),
			CONNECT_ONE_SHOT)
			...
			$"HBoxContainer/VBoxContainer/Button-Hoarder".add_child(DialogueButtonVar)
...
func _text_resource(i: DialogueText):
	...
	if camera and i.camera_position != Vector2(999.999, 999.999):
		...
		camera_tween.tween_property(camera, "global_position", i.camera_position, i.camera_transition_line)
	...
	while DialogueLabel.visible_characters < total_characters:
		...
		if character_timer > (1.0/i.text_speed) or text_without_square_brackets[DialogueLabel.visible_characters] == " ":
			...
			if character != " ":
				...
				if i.speaker_img_frames != 1:
					SpeakerSprite.frame < i.speaker_img_Hframes - 1
